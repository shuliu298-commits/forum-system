...
class PostApiIntegrationTest extends AbstractApiTest {
    ...
    void createPost() throws Exception {
        JsonNode resp = postJson("/api/posts",
    ...
    void createPostUnauthorized() throws Exception {
        JsonNode resp = postJson("/api/posts", Map.of("content", "游客发帖"), null);
    ...
    void listPosts() throws Exception {
        postJson("/api/posts", Map.of("content", "列表测试1"), token);
        postJson("/api/posts", Map.of("content", "列表测试2"), token);

        JsonNode resp = getJson("/api/posts?page=0&size=10", null);
        assertCode(resp, 0);
    ...
    void detailContainsComment() throws Exception {
        String postId = data(postJson("/api/posts", Map.of("content", "有评论的帖子"), token))
                .get("id").asText();
        createdPostIds.add(postId);

        JsonNode comment = postJson("/api/posts/" + postId + "/comments",
                Map.of("content", "第一条评论"), token);
        assertCode(comment, 0);
        String commentId = data(comment).get("id").asText();

        JsonNode detail = getJson("/api/posts/" + postId, null);
        assertCode(detail, 0);
    ...
    void commentValidation() throws Exception {
        String postId = data(postJson("/api/posts", Map.of("content", "校验帖"), token))
                .get("id").asText();
        createdPostIds.add(postId);

        assertCode(postJson("/api/posts/" + postId + "/comments", Map.of("content", "x"), null), 40100);
        assertCode(postJson("/api/posts/" + postId + "/comments", Map.of("content", ""), token), 40001);
    }

    @Test
    @DisplayName("T10 删除评论 -> 详情中不再出现")
    void deleteComment() throws Exception {
        String postId = data(postJson("/api/posts", Map.of("content", "删评论帖"), token))
                .get("id").asText();
        createdPostIds.add(postId);
        String commentId = data(postJson("/api/posts/" + postId + "/comments",
                Map.of("content", "待删除"), token)).get("id").asText();

        assertCode(deleteJson("/api/comments/" + postId + "/" + commentId, token), 0);
    ...
    void deleteOtherUsersPost() throws Exception {
        String ownPostId = data(postJson("/api/posts", Map.of("content", "别人的帖子"), token))
                .get("id").asText();
        ...
        assertCode(deleteJson("/api/posts/" + ownPostId, otherToken), 40300);
    }

    @Test
    @DisplayName("T14b 注销用户 -> 其帖子被清理")
    void deleteUserRemovesPosts() throws Exception {
        postJson("/api/posts", Map.of("content", "注销前的帖子"), token);

        assertCode(deleteJson("/api/users/" + userId, token), 0);

        JsonNode list = getJson("/api/posts?page=0&size=50", null);
